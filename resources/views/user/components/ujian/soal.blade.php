...
@elseif ($currentSoal->question && $currentSoal->image_content)
    <img id="gambarSoal" class="h-40 max-w-lg rounded-lg" src="{{ asset('storage/reading-ujian/' . $currentSoal->image_content) }}"
        alt="image">
    <p class="mt-4 font-normal">{!! nl2br(e($currentSoal->question)) !!}</p>
...
@elseif ($currentSoal->image_content && $currentSoal->question_image)
<img id="gambarSoal" class="h-44 max-w-lg rounded-lg" src="{{ asset('storage/reading-ujian/' . $currentSoal->image_content) }}"
alt="image">
<img class="h-30 mt-2 max-w-lg rounded-lg" src="{{ asset('storage/soal/' . $currentSoal->question_image) }}" alt="image">
